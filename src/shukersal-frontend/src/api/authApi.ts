...
export const authApi = {
  login: (
    credentials: LoginPostData
  ): Promise<ApiResponse<TokenResponseBody>> =>
    apiErrorHandlerWrapper(
      apiClient.post(`${serviceName}/login/`, credentials)
    ),
  register: (credentials: RegisterPostData): Promise<ApiResponse<Member>> =>
    apiErrorHandlerWrapper(
      apiClient.post(`${serviceName}/register/`, credentials)
    ),
};
